    ...
    for (size_t i = 0; i < num_blocks_vert; ++i) {
        for (size_t j = 0; j < num_blocks_hor; ++j) {
            ...
            for (const auto& prev_pair : prev_map) {
                ...
                int first = j * BLOCK_SIZE, second = i * BLOCK_SIZE + BORDER, count;
                count = kol;
                while(count) {
                    int pi = first, pj = second;
                    for(int ver = -1 - pas; ver < 2 + pas; ver++) {
                        for(int hor = -1 - pas; hor < 2 + pas; hor++) {
                            x = first + ver * count;
                            y = second + hor * count;
                            if(x >= 0 && y >= 0 && x < width && y < height){
                                const auto comp = prev + (y - second) * width_ext + x - first;
                                const int error = GetErrorSAD_16x16(cur, comp, width_ext);
                                if(error < best_vector.error){
                                    best_vector.x = x - first;
                                    best_vector.y = y - second;
