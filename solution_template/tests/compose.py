...
def compose_test(notebook_path, output_path):
    context = _read_context("tests/context.py")
    ...
    _write_test(context + '\n\n'.join([graded_source[name] for name in graded_source]), output_path)


def main():
    parser = argparse.ArgumentParser(description='Extract graded cells from notebook and compose output file')
    parser.add_argument('--notebook', type=str, default="motion_estimation.ipynb")
    parser.add_argument('--output', type=str, default="tests/src.py")
