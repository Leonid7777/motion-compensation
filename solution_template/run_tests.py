...
def test_equal():
    ...
    print(
        'SSIM {}\tPSNR {}'.format(
            ssim(frame, compensated_frame, multichannel=True),
            psnr(frame, compensated_frame)
        )
    )
...
def test_equal_shift_upper():
    ...
    print(
        'SSIM {}\tPSNR {}'.format(
            ssim(shifted_frame[1: height, :], compensated_frame[1: height, :], multichannel=True),
            psnr(shifted_frame[1: height, :], compensated_frame[1: height, :])
        )
    )
...
def test_equal_remap():
    ...
    print(ssim(compensated_frame_reference, compensated_frame))
